...
class DatabaseManager:
    def __init__(self, db_path=DB_FILE):
        """
        Maakt verbinding met de SQLite–database en initialiseert de tabellen.
        """
        ...
        try:
            ...
            self.connection.execute("PRAGMA journal_mode=WAL;")  # Zet database in Write-Ahead Logging modus
            self.cursor = self.connection.cursor()
            self._db_lock = threading.Lock()  # Lock voor concurrency
            logger.info(f"Verbonden met database: {self.db_path}")

            # Creëer / update tabellen indien nodig
            self.create_tables()
    ...
    def execute_query(self, query, params=(), retries=10, delay=0.2):
        ...
        """
        ## AANPASSING 1.3: Gebruik de lock in execute_query:
        with self._db_lock:
    ...
    def create_candles_table(self):
        ...
        try:
            ...
            self.connection.commit()
            logger.info("Candles table is klaar.")
        except Exception as e:
    ...
    def create_orderbook_tables(self):
        ...
        try:
            ...
            # Nieuwe index op (market, interval, timestamp)
            self.cursor.execute("""
    ...
    def alter_indicators_table(self):
        """
        Voegt (indien nodig) extra kolommen toe aan de bestaande 'indicators' tabel.
        """
        alter_statements = [
    ...
    def alter_trades_table(self):
        """
        Voegt (indien nodig) extra kolommen toe aan de bestaande 'trades' tabel:
         - position_id (TEXT)
         - position_type (TEXT)
         - status (TEXT)
         - pnl_eur (REAL)
    ...
    # Interne methode om de buffer in één keer te flushen
    def _flush_candle_buffer(self):
        ...
        try:
            ...
            # INSERT OR IGNORE INTO candles tijdelijk vervangen door INSERT OR REPLACE INTO CANDLES
        ...
        try:
            query = """
                ...
                VALUES (?, ?, ?, ?, ?,
                        ?, ?, ?, ?, ?)
            """
            # Haal uit trade_data wat je nodig hebt;
            # default = None of 0.0 als ze niet bestaan:
    ...
    def update_trade(self, trade_id: int, updates: dict):
        """
        Wijzig kolommen in trades (status, pnl_eur, fees, etc.) na partial close of exit.
        'updates' is een dict, vb. {'status': 'closed', 'pnl_eur': 12.34}
        """
        try:
            set_clauses = []
            params = []
    ...
    def prune_old_candles(self, days=30, interval=None):
        ...
        Als interval None is, worden ALLE intervals verwijderd.
        """
    ...
    def close_connection(self):
        ...
        if self.connection:
            ### AANPASSING 7: flush buffer voordat we de DB sluiten
            self.flush_candles()
            self.connection.close()
    ...
    def get_orderbook_snapshot(self, market: str):
        ...
        # Omzetten naar [[price, qty], [price, qty], ...]
