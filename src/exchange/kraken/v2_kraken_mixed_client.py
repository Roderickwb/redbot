...
# [QUEUE CHANGE] => Toevoegen voor onze events
import queue
...
class KrakenMixedClient:
    ...
    def __init__(self, db_manager, kraken_cfg: dict, use_private_ws=False):
        ...
        # [QUEUE CHANGE] => Maak hier de queue aan (voor partial fills)
    ...
    def _process_ohlc_data(self, data_list):
        ...
        if is_candle_closed(ts_ms, interval_str):
            ...
            logger.debug("[KrakenMixedClient] Candle voor %s op ts=%d nog niet gesloten; probeer REST-fallback.",
                         local_pair, ts_ms)
            # Tijdelijk niet gebruiken. Voor 15M poll zie _start_poll_15m_thread
    ...
    def _handle_own_trade(self, txid, fill_data):
        """
        We gooien de fill in self.trade_fills_queue i.p.v. direct DB calls =>
        concurrency-veiliger, en we verwerken het in Executor (zie executor.py).
        """
        ...
        # [QUEUE CHANGE] => push in queue. Originele DB-code uitcommentariëren.
        fill_event = {
            "txid": txid,
            "side": side,
            "vol": vol,
        ...
        # Originele DB-opslaan code => uitcommentariëren om concurrency te vermijden in WS-thread:
        """
        # fill_row = {
        #     "order_id": txid,
        #     "market": pair,
