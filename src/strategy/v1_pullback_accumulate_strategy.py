...
class PullbackAccumulateStrategy:
    ...
    def execute_strategy(self, symbol: str):
        ...
        if not df_entry.empty:
            ...
            # ws price (live):
            current_ws_price = self._get_ws_price(symbol)

            # AANPASSING #3: debug info (close(15m) vs ws_price)
            self.logger.debug(f"[Debug-15m] symbol={symbol}, last_close(15m)={current_price}, ws_price={current_ws_price}")
        ...
        if pullback_detected and not has_position:
            if direction == "bull":
                ...
                if (rsi_val >= self.rsi_bull_threshold
                    ...
                    self._open_position(symbol, side="buy", current_price=current_ws_price,
                ...
                if (rsi_val <= self.rsi_bear_threshold
                    ...
                    self._open_position(symbol, side="sell", current_price=current_ws_price,
                                        atr_value=atr_value, extra_invest=invest_extra_flag)
                    if invest_extra_flag:
                        self.invested_extra = True
